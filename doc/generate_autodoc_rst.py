...
def create_module_file(package, module, opts):
    ...
    text = format_heading(1, 'Módulo %s' % module)
    text += format_heading(2, 'Módulo :mod:`%s`' % module)
...
def create_package_file(root, master_package, subroot, py_files, opts, subs):
    ...
    text = format_heading(1, 'Paquete %s' % package)
    # add each package's module
    for py_file in py_files:
        if shall_skip(os.path.join(root, py_file), opts):
            continue
        is_package = py_file == INIT
        py_file = os.path.splitext(py_file)[0]
        py_path = makename(subroot, py_file)
        if is_package:
            heading = 'Paquete :mod:`%s`' % package
        else:
            heading = 'Modulo :mod:`%s`' % py_file
    ...
    if subs:
        text += format_heading(2, 'Subpaquetes')
        text += '.. toctree::\n\n'
        for sub in subs:
...
def create_modules_toc_file(master_package, modules, opts, name='modules'):
    ...
    """
    text = format_heading(1, 'Modulos %s' % opts.header)
    text += '.. toctree::\n'
    text += '   :maxdepth: %s\n\n' % opts.maxdepth
    ...
    for root, subs, files in tree:
        ...
        if INIT in py_files:
            ...
            if (# ... with subpackage(s)
                ...
                not shall_skip(os.path.join(root, INIT), opts)
               ):
            ...
            for py_file in py_files:
                if not shall_skip(os.path.join(path, py_file), opts):
